...
#' @description This function draws the visual predictive check plot with bootsraped data
...
bootVPC<-function(orig_data,
   ...
   bootTemp <- NULL
   IDlist = names(table(plot_data$Subject))
   ...
   for (j in 1:B){
     ...
     bootTemp = rbind(bootTemp,
                      tapply(BootSamp$Y,BootSamp$time.cut,
   ...
   SIM_quant = SIM_quant[,c(1:2,4)] %>% 
      spread(quantile,`50%`) %>% 
      left_join(time_bin$COVbin_summary[,c(1,6)],by="cut_temp") %>% 
      select(cut_temp,mid_LU,everything())
   ...
   if(plot_flag){
    ...
    return(list(DV_point=as_tibble(DV_point),
                Boot_CI=as_tibble(Boot_CI),
                SIM_quant=as_tibble(SIM_quant)))
  }
}
