...
#' @description  This function calculates percentiles of original data  and simulated data 
...
asVPC<-function(orig_data,
   ...
   main_title = paste0("average shifted VPC : ",weight_method,"-related weights")
   lower = upper = NA
   quantile=mid_COV=LB=UB=X=Y=Quant=Mid=cut_temp=mid_LU=X_bin=X_mid=NULL
      ...
      temp<-t(apply(temp.quantile,2,
                    function(x) 
                       stats::quantile(x,prob=c(alpha/2,0.5,1-alpha/2),
                                         na.rm=TRUE)))
      tempD = data.frame(cut_temp = 
                            as.character(time_bin$COVbin_summary$cut_temp[i]),
                         quantile = paste0("Q",as.character(probs*100),"th"),
                         temp)
      SQY = rbind(SQY,tempD)
      FQY = rbind(FQY,c(temp.orig.q))
   }
   ...
   TT=names(table(SQY$quantile))
   D1 = SQY %>% filter(quantile==TT[1]) %>% left_join(FQY,by="cut_temp") 
   D1 = D1[,c(1,3:5,10,11,8:9)]; 
   colnames(D1)[c(2:4,6)]= c("LB","Mid","UB","Quant")
   D2 = SQY %>% filter(quantile==TT[2]) %>% left_join(FQY,by="cut_temp") 
   D2 = D2[,c(1,3:5,10,12,8:9)]; 
   colnames(D2)[c(2:4,6)]= c("LB","Mid","UB","Quant")
   D3 = SQY %>% filter(quantile==TT[3]) %>% left_join(FQY,by="cut_temp") 
   ...
   if(plot_flag){
      ...
      QTemp=FQY[,c(1,6,7:9)] %>% 
         rename(X_bin=cut_temp,X_mid = mid_LU) %>% 
         select(X_bin,X_mid, everything())
      STemp=SQY %>% left_join(FQY[,c(1,6)],by="cut_temp") %>% 
         rename(X_bin=cut_temp,X_mid = mid_LU) %>% 
         select(X_bin,X_mid, everything())
      DV_point = orig_data[,c(X_name,Y_name)]
      return(list(DV_point=as_tibble(DV_point),
                  DV_quant=as_tibble(QTemp),
                  SIM_quantCI=as_tibble(STemp)))
  }
}
