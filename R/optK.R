...
optK<-function(X,
   ...
   N = length(X)
   if(is.null(maxK))
      maxK <- min(c(20,round(length(X)/10),length(table(X))))
   keep.tot<-NULL
   for(k in 3:maxK){
      ...
      if(bestCUT$WSS!=0){
        ...
        Phi = sum(unlist(lapply(bestCUT$cutoffs,function(xx)
                 ...
                 hk = ifelse(nk>1 & stats::sd(x)!=0 & timeDate::kurtosis(x)< C1,
                             hk/R,hk)
                 z=(xx-x)/hk
     ...
     keep.tot<-rbind(keep.tot,c(k,Jopt,KBratio))
   }
   ...
   if(Kmethod=="cluster"){
     ...
     K = min(keep.tot$k[which(keep.tot$KBratio==min(keep.tot$KBratio,na.rm=TRUE))])      
   }
