...
#' @description Discretise numeric data into categorical variable using 
#'  the user-defined breaks. If cutoffs and the number of bins (K) are NULL, 
#'  find the best number of bins using optK function and
#'  find the best cutoff values using FindBestCut function. 
#' @usage makeCOVbin(X,
#'            K,
#'            cutoffs,
#'            adjust0bin=TRUE,...)
...
makeCOVbin<-function(X,
   ...
   tab<-plyr::ddply(data_temp,plyr::.(cut_temp), plyr::summarise,
              mid_COV=round(stats::median(X, na.rm=T),2),.drop=FALSE)
