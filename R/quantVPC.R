...
#' of density or shape of the observed data distribution, 
#' through the form of percent.
...
#' data(origdata)
#' data(simdata)
#' quantVPC(origdata,simdata,prob=0.5)
#' }
...
quantVPC<-function(orig_data,
  ...
  if(is.null(N_xbin))
   ...
   temp.sim.Q$Q50th = unlist(temp.sim.Q2[,4])
   QVPC.temp <- NULL
   ...
   for(i in 1:nrow(temp.sim.Q)){
     ...
     above <- sum(temp$Y>Mt)/N*100
     below <- sum(temp$Y<=Mt)/N*100
     unavail <- 100-(above+below)
     obsmid <- prob*100+unavail/2
     QVPC.temp <- rbind(QVPC.temp,c(midtime,N,Mt,obsmid,above,below,unavail))
  }

  QVPC.temp<-as.data.frame(QVPC.temp)
  colnames(QVPC.temp)<-c("TIME","N","Mt","obsmid","above","below","unavailable")

  QVPC.temp %>% gather(key="type",value="percent",above:unavailable) %>%
    ggplot()+
